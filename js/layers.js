...
addLayer("b", {
        ...
        hotkeys: [
            {key: "u", description: "按 U 进行UN器重置。", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
        ],
		...
		effectDescription() {
			return "增幅字符获取 "+format(tmp.b.effect)+"x"+(tmp.nerdMode?(inChallenge("ne", 11)?"\n (禁用)":("\n(每个 "+format(tmp.b.effectBase)+"x)")):"")
		},
		doReset(resettingLayer) {
			let keep = [];
...
addLayer("s", {
        ...
        color: "#57d4c6",
        requires() { return new Decimal(1e120).times(Decimal.pow("1e180", Decimal.pow(player[this.layer].unlockOrder, 1.415038))) }, // Can be a function that takes requirement increases into account
        resource: "空间量化", // Name of prestige currency
        baseResource: "字符", // Name of resource prestige is based on
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		...
		tabFormat: ["main-display",
			...
			"blank",
			["display-text",
				function() {return '你最多拥有 ' + formatWhole(player.s.best) + ' 空间量化'},
					{}],
			"blank",
			"milestones", "blank", 
			["display-text",
				function() {return '你有 ' + format(player.g.power) + ' KP'},
					{}],
			["display-text",
				function() {return '你的空间量化为你提供了 ' + formatWhole(tmp.s.space) + ' 通道'},
					{}],
			["display-text",
				function() {return tmp.s.buildingPower.eq(1)?"":("通道增益: "+format(tmp.s.buildingPower.times(100))+"%")},
					{}],
			"blank",
		...
		upgrades: {
			...
			15: {
				title: "弯道",
				description: "<b>第四ROM通道</b> 成本开立方根，3x 弱秒，并增益 <b>查找字符串</b> （效果是 2.7 次方根）。",
			...
			23: {
				title: "反转空间",
				description() { return (player.e.unlocked&&player.t.unlocked&&(player.s.unlockOrder||0)==0)?"所有通道价格除以 1e20。":("空间以你首先选择空间的方式运行"+(player.t.unlocked?"，并且所有通道价格除以 1e20。":"。")) },
			...
			31: {
				title: "通道开拓",
				description: "前四个通道的价格指数降低 0.04*(5-n)，n 是这个通道的编号。",
		...
		buyables: {
			...
			11: {
				...
				display() { // Everything else displayed in the buyable button after the title
                   ...
                   "+(tmp.nerdMode?("公式: 等级^sqrt(空间能量)*等级*4"):(" 空间能量加字符和子串级获取 " + format(data.effect) +"x"))
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
			...
			12: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    return (tmp.nerdMode?("价格公式: "+format(tmp.s.buildingBaseCosts[this.id])+"^((x*"+format(tmp.s.buildScalePower)+")^"+format(tmp[this.layer].buyables[this.id].costExp)+")*"+format(tmp.s.buildingBaseCosts[this.id])+"/"+format(tmp.s.divBuildCosts)):("价格: " + formatWhole(data.cost) + " KP"))+"\n\
                    等级: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
                    "+(tmp.nerdMode?("公式: sqrt(等级)"):("加成UN器和键盘软垫底数 +" + format(data.effect)))
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                    return player.g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.s.space().gt(0)},
...
addLayer("sb", {
		...
		tabFormat: ["main-display",
			"prestige-button",
			"blank",
			["display-text", function() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("sb"):false)?("你的ACD器为你提供了 <h3 style='color: #71ffc1; text-shadow: #b7fff0 0px 0px 10px;'>"+formatWhole(tmp.sb.spectralTotal)+"</h3> casU"+(tmp.nerdMode?(" (每个 "+formatWhole(tmp.sb.spectralEach)+")"):"")+"，计算入UN器效果，但不计入UN器相关的升级效果。"):"" }],
		],
		startData() { return {
			unlocked: false,
			points: new Decimal(0),
...
*/
addLayer("sg", {
        name: "Kbreplicator", // This is optional, only used in a few places, If absent it just uses the layer id.
        symbol: "KBr", // This appears on the layer's node. Default is the id with the first letter capitalized
        position: 4, // Horizontal position within a row. By default it uses the layer id and sorts in alphabetical order
        color: "#8fc6bb",
        requires: new Decimal(200), // Can be a function that takes requirement increases into account
        resource: "键盘复制", // Name of prestige currency
        baseResource: "键软", // Name of resource prestige is based on
        ...
        row: 2, // Row the layer is in on the tree (0 is the first row)
        hotkeys: [
            {key: "K", description: "按 Shift+K 进行键盘复制重置。", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
        ],
		...
		effectDescription() {
			return "生成 "+format(tmp.sg.effect)+" infitKP/sec"+(tmp.nerdMode?("\n (每个 "+format(tmp.sg.effectBase)+"x)"):"")
		},
		enEff() {
			if (!unl(this.layer)) return new Decimal(1);
		...
		tabFormat: ["main-display",
			...
			"blank",
			["display-text",
				function() {return '你有 ' + format(player.sg.power) + ' infitKP，增幅键软底数 '+format(tmp.sg.enEff)+'x'+(tmp.nerdMode?(" (sqrt(x+1))"):"")},
					{}],
			"blank",
			["display-text", function() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("sg"):false)?("你的键盘复制为你供了 <h3 style='color: #63fcff; text-shadow: #a3ffec 0px 0px 10px;'>"+formatWhole(tmp.sg.spectralTotal)+"</h3> 虚键软"+(tmp.nerdMode?(" (((log(timeSinceRow4Reset+1)*(SG^2))^0.95)*1.2)"):"")+"，计算入键软效果，但不计入键软相关的升级效果。"):"" }],
		],
		startData() { return {
			unlocked: false,
			points: new Decimal(0),
...
*/
addLayer("h", {
        name: "EpsilonParticle", // This is optional, only used in a few places, If absent it just uses the layer id.
        ...
        resource: "对虚粒子", // Name of prestige currency
        ...
        hotkeys: [
            {key: "e", description: "按 E 进行粒子重置。", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
        ],
        doReset(resettingLayer){ 
			let keep = [];
		...
		milestones: {
			0: {
				...
				requirementDescription: "重复生产--e300,000,000 虚粒子",
				done() { return player.h.points.gte("e3e8") },
				effectDescription: "解锁自动可重复生成粒子。",
				toggles: [["h", "auto"]],
			},
		},
		challenges: {
			...
			11: {
				...
				rewardDescription: "解锁透析升级，同时虚粒子的效果提升至 1.2 次幂。",
			...
			12: {
				name: "时弱粒子",
				completionLimit: 1,
				challengeDescription: "UN器/键软底数被时间消减（你的ACD器会放大此效果）。",
			...
			22: {
				name: "虚字符粒子",
				completionLimit: 1,
				challengeDescription: "只有子串级升级、成就奖励和第一通道能增益字符获取。",
			...
			31: {
				...
				challengeDescription() {
					...
					return "你只能买 10 个虚弱后继和事件切片（总计），虚弱后继/事件切片自动购买已被禁止，同时字符生成被开 "+format(tmp.h.pointRoot31)+" 次根。<br>完成次数: "+formatWhole(challengeCompletions("h", 31))+(infLim?"":("/"+lim));
				},
				...
				currencyDisplayName: "字符",
				currencyInternalName: "points",
				rewardDescription() { return "<b>虚后继粒子</b> 加成infitKP 获取，基于"+(hasUpgrade("ss", 33)?"当前游戏时间。":"当前第四行重置后时间。") },
				rewardEffect() { 
			...
			32: {
				...
				challengeDescription() { 
					let lim = this.completionLimit();
					let infLim = !isFinite(lim);
					return '之前的所有粒子一起生效（"虚后继粒子" 以第 '+formatWhole(challengeCompletions("h", 32)*2+1)+' 级难度生效)<br>完成次数: '+formatWhole(challengeCompletions("h", 32))+(infLim?"":('/'+lim))
				},
				goal() {
				...
				currencyDisplayName: "字符",
				currencyInternalName: "points",
				rewardDescription: "<b>全虚粒子</b> 完成次数加成 TE 获取。",
				rewardEffect() { 
			...
			41: {
				name: "重置增粒子",
				completionLimit: 1,
				challengeDescription: "进行一次第五行重置，X粒体和Y粒体都重置了，并且其惩罚被夸张地放大。",
...
*/
addLayer("q", {
        name: "intangible", // This is optional, only used in a few places, If absent it just uses the layer id.
        ...
        resource: "透术", // Name of prestige currency
        baseResource: "KP", // Name of resource prestige is based on
        ...
        row: 3, // Row the layer is in on the tree (0 is the first row)
        hotkeys: [
            {key: "i", description: "按 I 进行透析重置。", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
        ],
        doReset(resettingLayer){ 
			let keep = [];
			player.q.time = new Decimal(0);
		...
		tabFormat: {
			"Main Tab": {
				content: [
					...
					"blank",
					["display-text",
						function() {return '你有 ' + formatWhole(player.g.power)+' KP'},
							{}],
					["display-text",
						function() {return '你最多拥有 ' + formatWhole(player.q.best)+' 透术'},
							{}],
					["display-text",
						function() {return '你总共拥有 ' + formatWhole(player.q.total)+' 透术'},
							{}],
					"blank",
					["display-text",
						function() {return '你有 ' + formatWhole(player.q.energy)+' IE ('+(tmp.nerdMode?('基础获取: (timeInRun^(quirkLayers-1))'):'由光片生成')+')，增幅字符和 KP 获取 ' + format(tmp.q.enEff)+(tmp.nerdMode?(" ((x+1)^"+format(hasUpgrade("q", 23)?6:2)+"*"+format(improvementEffect("q", 23))+")"):"")},
							{}],
					"blank",
					"milestones", "blank",
					"blank",
					"buyables", "blank",
					["display-text", "注意: 大部分透明升级随时间变贵，但在执行透明重置时恢复。"], "blank",
					"upgrades"],
			},
			Improvements: {
				unlocked() { return hasUpgrade("q", 41) },
				buttonStyle() { return {'background-color': '#a7c5ff'} },
				content: [
					"main-display",
					"blank",
					["display-text",
						function() {return '你有 ' + formatWhole(player.q.energy)+' IE ('+(tmp.nerdMode?('基础获取: (timeInRun^(quirkLayers-1))'):'由光片生成')+'), 提供了下列光谱调整 (下一个需要 '+format(tmp.q.impr.overallNextImpr)+')'},
							{}],
					"blank",
					"improvements"],
			},
		...
		buyables: {
			...
			11: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    let display = (tmp.nerdMode?("价格公式: "+format(data.costBase)+"^("+format(data.costBase)+"^x-1)"):("价格: " + formatWhole(data.cost) + " 透术")+"\n\
		...
		milestones: {
			0: {
				requirementDescription: "20%透明--2 总透术",
				done() { return player.q.total.gte(2) || hasAchievement("a", 71) },
				effectDescription: "对任何重置保留UN器、键软、空间和时间里程碑。",
			},
			1: {
				requirementDescription: "光焦三层--3 总透术",
				done() { return player.q.total.gte(3) || hasAchievement("a", 71) },
				effectDescription: "你可以最大购买时间和空间，每秒获得 100% 弱秒，并解锁自动虚弱后继和自动事件切片。",
				toggles: [["e", "auto"], ["t", "autoExt"]],
			},
			2: {
				requirementDescription: "重新聚焦--4 总透术",
				done() { return player.q.total.gte(4) || hasAchievement("a", 71) },
				effectDescription: "对任何重置保留空间、虚弱和时间升级，同时在透析/粒子重置中保留通道。",
			},
			3: {
				requirementDescription: "透光获取--6 总透术",
				done() { return player.q.total.gte(6) || hasAchievement("a", 71) },
				effectDescription: "解锁自动时间层面和自动空间量化。",
				toggles: [["t", "auto"], ["s", "auto"]],
			},
			4: {
				requirementDescription: "双生层级--10 总透术",
				done() { return player.q.total.gte(10) || hasAchievement("a", 71) },
				effectDescription: "解锁障碍和自动超级增幅器。",
				toggles: [["sb", "auto"]],
			},
			5: {
				requirementDescription: "聚焦交换--25 总透术",
				done() { return player.q.total.gte(25) || hasAchievement("a", 71) },
				effectDescription: "时间、空间和ACD器不再重置任何东西，同时你可以摧毁通道。",
			},
			6: {
				unlocked() { return player.sg.unlocked },
				requirementDescription: "生成革命-1e22 总透术",
				done() { return player.q.total.gte(1e22) || hasAchievement("a", 71) },
				effectDescription: "解锁自动软垫复制，并且软垫复制不再重置任何东西。",
				toggles: [["sg", "auto"]],
			},
			7: {
				unlocked() { return player.sg.unlocked },
				requirementDescription: "完全第三--1e60 总透术",
				done() { return player.q.total.gte(1e60) || hasAchievement("a", 71) },
				effectDescription: "你可以最大购买ACD器和软垫复制，同时解锁自动通道。",
				toggles: [["s", "autoBld"]],
			},
		},
		upgrades: {
			...
			11: {
				title: "反透光片",
				description: "总透术加成光派生产（由透析升级数量求幂）。",
			...
			22: {
				title: "免征芯片",
				description: "透术提供免费的事件切片、虚弱后继和通道。",
			...
			24: {
				title: "第一效升芯",
				description: "TE 的第一个效果和虚弱后继的第一个效果被提升到 7.5 次幂。",
			...
			31: {
				title: "环形增长晶体管",
				description: "基于光片，从 12 延缓 2/3 静态比例的软上限。",
			...
			34: {
				title: "UN仿生晶体管",
				description: "任何增加UN器底数的东西都会以较低的比例对其做乘法。",
			...
			35: {
				...
				currencyLayer: "q",
				pseudoUnl() { return player.i.buyables[12].gte(6) },
				pseudoReq: "需要: 无透光达到 e5,000,000 IE（使用第五行重置）。",
...
*/
addLayer("m", {
		name: "efficacy", // This is optional, only used in a few places, If absent it just uses the layer id.
        symbol: "Eff", // This appears on the layer's node. Default is the id with the first letter capitalized
        position: 1, // Horizontal position within a row. By default it uses the layer id and sorts in alphabetical order
        startData() { return {
            unlocked: false,
			points: new Decimal(0),
        ...
        resource: "能效", // Name of prestige currency
        baseResource: "虚粒子", // Name of resource prestige is based on
...
*/
addLayer("ba", {
		name: "clash", // This is optional, only used in a few places, If absent it just uses the layer id.
        ...
        resource: "氦核", // Name of prestige currency
        baseResource: "透术", // Name of resource prestige is based on
		...
		bars: {
			balanceBar: {
				...
				fillStyle() { 
					let r = 92 + (90 - 92) * tmp.ba.bars.balanceBar.progress;
					let g = 187 + (254 - 187) * tmp.ba.bars.balanceBar.progress;
					let b = 255 + (123 - 255) * tmp.ba.bars.balanceBar.progress;
		...
		upgrades: {
			...
			12: {
				title: "阳离子",
				description: "Y粒体加成通道效果和所有第二空间效果。",
				cost() { return new Decimal(player.ma.current=="ba"?"1e166666":5e7) },
				currencyDisplayName: "Y粒体",
			...
			21: {
				title: "Z粒体",
				description: "对虚粒子的效果提升至 8 次幂。",
				cost() { return new Decimal(player.ma.current=="ba"?"1e189500":25e7) },
				unlocked() { return hasUpgrade("ba", 13)&&hasUpgrade("ba", 14) },
			},
			22: {
				title: "XX粒体",
				description: "X粒体同样加成对虚粒子和透术获取。",
			...
			24: {
				...
				effectDisplay() { return "Y: "+format(tmp.ba.upgrades[24].effect.pos)+"x, X: "+format(tmp.ba.upgrades[24].effect.neg)+"x" },
				formula() { return "Y: (log(neg/1e12+1)+1)^"+(hasUpgrade("ba", 33)?15:5)+", X: (log(pos/1e12+1)+1)^"+(hasUpgrade("ba", 33)?15:5) },
				style: {"font-size": "9px"},
			},
			31: {
				title: "公式粒体 ",
			...
			33: {
				title: "双重净化",
				description: "<b>奖励惩罚</b> 的两个效果提升至三次方。",
		...
		milestones: {
			0: {
				requirementDescription: "秒透术--2 总氦核",
				done() { return player.ba.total.gte(2) || (hasMilestone("hn", 0)) },
				effectDescription: "每秒获得 100% 透术，对所有重置保留透析升级。",
			},
			1: {
				requirementDescription: "自光层--3 总氦核",
				done() { return player.ba.total.gte(3) || (hasMilestone("hn", 0)) },
				effectDescription: "解锁自动光片。",
				toggles: [["q", "auto"]],
			},
			2: {
				requirementDescription: "非二空间能量--10 总氦核",
				done() { return player.ba.total.gte(10) || (hasMilestone("hn", 0)) },
				effectDescription: "对任何重置保留第二空间升级，解锁自动第二空间能量，第二空间能量不再重置任何东西。",
				toggles: [["ss", "auto"]],
			},
			3: {
				unlocked() { return hasMilestone("ba", 2) },
				requirementDescription: "升级--200,000 总氦核",
				done() { return player.ba.total.gte(2e5) || (hasMilestone("hn", 0)) },
				effectDescription: "解锁碰撞升级。",
			},
			4: {
				unlocked() { return hasMilestone("ba", 3) },
				requirementDescription: "重置保留--1e12 总氦核",
				done() { return player.ba.total.gte(1e12) || (hasMilestone("hn", 0)) },
				effectDescription: "你可以在重置时保留X粒体和Y粒体。",
